...
describe("applyZedSettings", () => {
	test("writes non-none reasoning_effort and anthropic adaptive mode", () => {
		const next = applyZedSettings(
			`{"language_models":{"openai_compatible":{"Hadrian - OpenAI":{}},"anthropic_compatible":{"Hadrian - Anthropic":{}}}}`,
		...
		const leftover = parsed.language_models.openai_compatible["Hadrian - OpenAI"].available_models[0];
