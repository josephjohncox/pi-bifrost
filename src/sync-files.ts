...
export function applyZedSettings(source: string, catalog: BuiltCatalog, config: BifrostConfig): string {
	...
	assignProvider(openaiCompatible, openaiName, openaiProvider, /hadrian/i);
	assignProvider(anthropicCompatible, anthropicName, anthropicProvider, /hadrian/i);
...
function assignProvider(
	...
	for (const key of Object.keys(map)) {
		if (key !== name && legacy.test(key)) map[key] = provider;
	}
}
